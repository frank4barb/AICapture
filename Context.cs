...
namespace AICapture
{
    ...
    public class Context
    {
        ...
        //restituisce la directory di progetto (solo per debug)
        private static string projectDirectory()
        {
            string path = Environment.CurrentDirectory;
            string[] parts = path.Split(Path.DirectorySeparatorChar);
            int binIndex = Array.FindIndex(parts, part => part.Equals("bin", StringComparison.OrdinalIgnoreCase));
            if (binIndex >= 0)
            {
                string[] resultParts = parts.Take(binIndex).ToArray();
                return string.Join(Path.DirectorySeparatorChar.ToString(), resultParts);
            }
            return path; // Se "bin" non è trovato, restituisce il percorso originale
        }
        ...
        public static Context Instance
        {
            get
            {
                if (_instance == null)
                {
                    ...
                    // check INI file
                    if (!File.Exists(_instance.PathIniFile)) { _instance.PathIniFile = _instance.ProjectDirectory + "\\..\\" + "CTXdesktop.ini"; }
